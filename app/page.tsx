...
import {ArrowUpRight,BookOpen,CalendarDays,Cloud,Gem,Grid3X3,Sparkles,Table2} from 'lucide-react';
...
const sources=[
 ...
 {slug:'tembo',index:'05',name:'Tembo',kind:'Cloud agents',description:'Run, automate, and review coding-agent sessions in isolated cloud environments.',Icon:Cloud,tone:'blue'},
 {slug:'profound',index:'06',name:'Profound',kind:'AI marketing',description:'Answer-engine visibility, AI Marketer workflows, citations, prompts, and analytics.',Icon:Gem,tone:'black'},
 {slug:'agentic-sales',index:'07',name:'Agentic Sales',kind:'Internal prototype',description:'A sourced command center for sales agents, signals, approvals, workflows, and pipeline.',Icon:Sparkles,tone:'acid'},
];
export default function Home(){return <main className={styles.page}><header><div><span className={styles.eyebrow}>INSPIRATION / INDEX</span><h1>Source material,<br/>made operational.</h1></div><div className={styles.intro}><p>Interface studies translated into adaptable components and Storybook stories. Library primitives first. Bespoke only where the source demands it.</p><Link href="/storybook"><BookOpen size={15}/>Open Storybook</Link></div></header><section className={styles.grid}>{sources.map(({slug,index,name,kind,description,Icon,tone})=><Link href={`/inspirations/${slug}`} className={`${styles.card} ${styles[tone]}`} key={slug}><div className={styles.cardTop}><span>{index}</span><Icon/></div><div><small>{kind}</small><h2>{name}</h2><p>{description}</p></div><span className={styles.enter}>Marketing + product <ArrowUpRight size={15}/></span></Link>)}</section><footer><span>VERCEL / NEXT.JS</span><span>STORYBOOK 10</span><span>6 SOURCES + 1 PROTOTYPE</span></footer></main>}
